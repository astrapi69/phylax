...
- [ ] **I18N-01l** Extract remaining common + errors strings (also: audit fuer/für umlaut-vs-ASCII inconsistency across all namespaces incl. new Grösse/Groesse case, and decide normalization strategy; strong hoist candidates: `action.cancel/back/next`, `counts.*` from profile-list + profile-import, entity section headings + field labels from ai-chat duplicates with observations/lab-values/supplements, supplement category lowercase triple-duplicate)
