...
- [ ] **EntryRouter + ProtectedRoute auth-decision duplication** - both compute the same "no meta / locked / unlocked" decision. Extract `resolveAuthDestination()` helper in `src/router/` and have both call it. Low-risk refactor, nontrivial since EntryRouter is async and ProtectedRoute subscribes to lock changes.
...
- [ ] **I18N-01m** (superseded by I18N-02-e below)
...
- [ ] **I18N-02-b** Content EN translations: observations, lab-values, supplements, open-points, timeline, profile-view, profile-list, profile-create, export. ~122 strings + plural conversions for count-bearing keys
- [ ] **I18N-02-c** ai-config + donation + import EN translations. ~136 strings + plural conversions
- [ ] **I18N-02-d** ai-chat EN translations. ~150 strings + plural conversions (largest namespace; split from 02-c for reviewability)
- [ ] **I18N-02-e** Go-live: custom detector (`src/i18n/detector.ts`, no npm package), LanguageSection component, settings.language.heading/auto/option keys, detector + LanguageSection test suites, flip `LANGUAGE_SWITCHER_ENABLED` to true. Auto / Deutsch / English with localStorage persistence at `phylax-language`
