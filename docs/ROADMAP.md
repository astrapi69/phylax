...
- [ ] **SetupFlowGuard unlocked-redirect to /profile** - SetupFlowGuard currently redirects authenticated users to `/unlock` when they direct-link to `/welcome`, `/privacy`, or `/setup`, forcing re-authentication. With `resolveAuthState()` in hand the fix is one line: map `unlocked` to `/profile` instead. Behavior improvement, not refactor; deferred from TD-06 for scope discipline.
