...
- [ ] **I18N-01l-c** Create common namespace + hoist cross-namespace duplicates (≥3 threshold): `common.error.no-profile` (7 sources), `common.action.cancel` (3), `common.action.close` (3), `common.counts.observations` (3), `common.counts.supplements` (3), `common.entity.open-points` (4 after 01l-b), `common.entity.ai-assistant` (3 after 01l-b), plus `common.status.loading` as semantic-equivalence exception for 2-namespace router loading state. Final verification grep sweep confirming zero hardcoded German in src/ outside locales + documented exceptions
