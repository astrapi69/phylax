...
- [ ] **German transliteration in non-locale source code (TD-09)** - TD-08 normalized DE locale files + glossary to Unicode umlauts, but several source files still emit transliterated German. Three sub-items with different fix strategies: (a) parser input keys in `parseGermanDate.ts`, `parseBasisdaten.ts`, `parseVersionshistorie.ts`, `parseProfile.ts` need backward-compatible lookup (real ePA exports may use either transliterated or Unicode forms; map both). (b) AI prompts in `promptFragments.ts`, `systemPrompt.ts` are straightforward Unicode rewrite. (c) Export labels in `markdownExport.ts` are straightforward rewrite. Deferred from TD-08 for scope discipline.
- [ ] **CI hardening for Playwright system-deps install (TD-10)** - CI E2E (Dev) job flaked twice in a row on Ubuntu mirror apt stall during `npx playwright install-deps chromium` (cb6d691 CI run, initial + rerun both hit 15-min timeout at `azure.archive.ubuntu.com` InRelease fetches). TD-04's `timeout-minutes: 15` correctly killed each runaway; without it each flake would have consumed the 6-hour default. Four sub-items: (a) add `-o Acquire::Retries=3 -o Acquire::http::Timeout=30` to the apt invocation so mirror stalls recover instead of eating the job timeout. (b) Cache system deps or at minimum make the install resilient; asymmetry with the working browser-binary cache is the actual bug. (c) Consider `--no-install-recommends` or a pinned alternative mirror to cut apt surface. (d) Upgrade `actions/checkout@v4`, `actions/setup-node@v4`, `actions/cache@v4` to their `@v5` equivalents once Node 24-compatible versions are available (GitHub forces Node 24 on 2026-06-02, removes Node 20 on 2026-09-16). Not urgent; prioritize when flakes become noisy or before the Node-20 removal window closes.
