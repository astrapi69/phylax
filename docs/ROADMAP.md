...
- [ ] **Playwright config drift prevention** - `playwright.config.ts` (dev) and `playwright.config.production.ts` (prod smoke) share critical settings (locale, retries, trace, screenshot). B-02 pinned `locale: 'de-DE'` in the dev config but missed the production config; the next CI run produced 92 failing production tests. A follow-up commit restored parity. Options to prevent the next drift: extract a shared base config imported by both, or add a CI-time diff check that fails when key fields disagree. Revisit if a third config is added or if another drift incident occurs.
