...
- [ ] **P-19** Search match highlighting in observation results. O-17 ships in-memory search across `theme + fact + pattern` without highlighting matched substrings; users see "X von Y Beobachtungen" but have to re-read each visible entry to find their match. Wrap the matched substrings in `<mark>` (or styled span) within `theme`, `fact`, and `pattern` rendering on the observation card while a search query is active. Watch points: needs careful diff/replace that respects existing Markdown rendering in `fact` / `pattern` (Markdown components render to nested elements; naive string replace would break formatting). Bundle-aware: a small custom highlighter is preferable to pulling in a library. Re-evaluate priority in Phase 7 polish or earlier if user feedback demands.
