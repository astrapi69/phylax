...
- [ ] **DOC-01** Iteration 1: minimum viable user documentation
  - [x] **DOC-01a** Tooling setup: clone Bibliogon-shape MkDocs scaffold into the new `phylax-docs` repo (mkdocs.yml at root, `docs_dir: docs/help`, `docs/help/de/` + `docs/help/en/` placeholders, `_meta.yaml` nav source, `scripts/generate_mkdocs_nav.py`, GitHub Pages CI workflow). Theme: Material indigo (Bibliogon default; brand decision deferred). Empty content pages, structured navigation. Verify deploy to https://astrapi69.github.io/phylax-docs/ succeeds and Phylax-PWA deploy pipeline stays unaffected. (Shipped 2026-04-27 in phylax-docs commit `6148826`. DE root + EN subpath both 200; PWA unaffected; `mkdocs build --strict` green in CI.)
