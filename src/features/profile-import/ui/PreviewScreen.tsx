...
export function PreviewScreen({
  ...
  return (
    <div>
      <h1 className="mb-4 text-xl font-bold text-gray-900">Vorschau</h1>

      <dl className="mb-4 grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
        <div>
          <dt className="inline font-medium text-gray-700">Quelle:</dt>{' '}
          <dd className="inline text-gray-900">{sourceLabel}</dd>
        </div>
        <div>
          <dt className="inline font-medium text-gray-700">Ziel:</dt>{' '}
          <dd className="inline text-gray-900">{targetProfileName}</dd>
        </div>
      </dl>

      <section className="mb-4 rounded border border-gray-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-semibold text-gray-900">Inhalt</h2>
      ...
      <section className="mb-4">
        ...
        {!hasWarnings && !hasUnrecognized && (
          <p className="text-sm text-green-700" data-testid="parse-clean">
          ...
          <Collapsible summary={`Offene Punkte anzeigen (${counts.openPoints})`}>
            <ul className="mt-2 space-y-1 text-sm text-gray-700">
              {parseResult.openPoints.map((p, i) => (
                <li key={i}>
                  <span className="text-gray-500">[{p.context}]</span> {truncate(p.text, 80)}
                </li>
              ))}
            </ul>
          </Collapsible>
...
function Collapsible({
  ...
  return (
    <div className="rounded border border-gray-200 bg-white" data-testid={testId}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
      ...
      >
        {icon && (
          <span
            className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-xs ${iconClass ?? 'bg-gray-100 text-gray-700'}`}
            aria-hidden
          >
            {icon}
          </span>
      ...
      {open && <div className="border-t border-gray-100 px-3 pb-3">{children}</div>}
    </div>
  );
}
