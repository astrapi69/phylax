...
describe('formatBackupFilename', () => {
  it('formats a known date as befaro-backup-YYYYMMDD-HHmmss.phylax', () => {
    const d = new Date(2026, 3, 21, 17, 30, 12); // local time, April 21 2026 17:30:12
    expect(formatBackupFilename(d)).toBe('befaro-backup-20260421-173012.phylax');
  });

  it('zero-pads single-digit month, day, hour, minute, second', () => {
    const d = new Date(2026, 0, 3, 5, 7, 9);
    expect(formatBackupFilename(d)).toBe('befaro-backup-20260103-050709.phylax');
  });
  ...
  it('defaults to the current wall-clock time', () => {
    ...
    expect(name).toMatch(/^befaro-backup-\d{8}-\d{6}\.phylax$/);
  });
});
