...
- **SEO and social-sharing metadata** [D-04]. The PWA shipped with
  ...
  previews. Added to `index.html`: primary SEO tags (description,
  keywords, author, robots, canonical), Open Graph and Twitter Card
  ...
  `en_US` locale pair. New static assets in `public/`: two
  dark-theme Open Graph cards (1200x630, DE + EN, generated by
  `scripts/generate-og-images.mjs` via the existing resvg
  pipeline, `make og-images`), an allow-all `robots.txt` with a
  sitemap reference (replacing the prior `Disallow: /`), and a
  minimal `sitemap.xml` listing only the crawlable landing route.
  Per-route meta, dynamic OG images, and pre-rendering are out of
  scope (social crawlers do not run JS; static tags suffice).
  Post-deploy crawler validation tracked as a one-time smoke:
  [`d-04-seo-social-metadata.md`](docs/manual-smoke/d-04-seo-social-metadata.md).
  No ADR (no architectural shift, zero bundle delta on JS/CSS).
...
- First test suite for `features/documents/useLinkableEntities.ts`
  ...
  over-40-char ellipsis path on `formatObservationLabel`, the
