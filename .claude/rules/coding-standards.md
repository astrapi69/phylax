...
Dev: `vite`, `@vitejs/plugin-react`, `vite-plugin-pwa`, `typescript`, `vitest`, `@vitest/coverage-v8`, `@testing-library/react`, `@testing-library/jest-dom`, `@testing-library/user-event`, `fake-indexeddb`, `jsdom`, `@playwright/test`, `@axe-core/playwright`, `@tailwindcss/typography` (ADR-0008), `size-limit`, `@size-limit/preset-app` (ADR-0010), `eslint`, `prettier`, `husky`, `lint-staged`.
